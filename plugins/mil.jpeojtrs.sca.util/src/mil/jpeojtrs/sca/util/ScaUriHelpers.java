...
public class ScaUriHelpers {
	...
	public static Resource getLocalFileResource(final String localFilePath, final EObject reference, final String targetFileSystem) {
		...
		try {
			...
			// Resource failed to demand load (see ResourceSet.getResource(URI, boolean))
			return null;
		} catch (RuntimeException e) { // SUPPRESS CHECKSTYLE The EMF JavaDoc says this may be thrown
			// We only expect this if "A resource can't be demand created" (see ResourceSet.getResource(URI, boolean))
	...
	/**
	 * Safely retrieves/loads a {@link Resource} from a {@link ResourceSet} using a lock to control reads/writes.
	 * The lock may be specified using {@link #RESOURCE_SET_LOCK}. This method throws exceptions generated by demand
	 * loading using {@link ResourceSet#getResource(URI, boolean)}.
	 * @since 5.1
	 */
	public static Resource loadResource(ResourceSet resourceSet, URI uri) {
		...
		final Resource tmpResource = tmpResourceSet.getResource(uri, true);
